<?php
  ...
  function printPeriod($courses, $period=1, $day_list, $mysqli){ 
    ...
    $subjects_count = $mysqli->query("SELECT COUNT(subject) as count, subject as id FROM course, term WHERE period={$period} AND type=1 AND course._year=term.year AND course._term=term.term AND term.current=1 GROUP BY subject");
    $subjects = [];
    ...
    $subject_box = ($period + 1)%2 + 1;
    for($i = 1; $i<=6; $i++){
      echo "<li>";
      echo "<div class='collapsible-header center ' style='border: none' subject-box='$subject_box' subject-id='$i'>
...
<div class="container">
  <h2>คอร์สปกติ</h2>
  <p>นักเรียนจะต้องสมัครเรียนทั้งคอร์ส
    1. คอร์ส<?php echo ($_SESSION['term'][0] == '1') ? 'กุมภาพันธ์ - มีนาคม' : 'กันยายน - ตุลาคม' ?> และ
    2. คอร์ส<?php echo ($_SESSION['term'][0] == '1') ? 'เทอม 1' : 'เทอม 2' ?>
    ซึ่งจะต้องสมัครทั้ง 2 คอร์สนี้ ใน 1 ครั้งการสมัคร
  </p>
  ...
  <div class="row">
    <div class="col s12 m6">
      <h5>คอร์ส <?php echo ($_SESSION['term'][0] == '1') ? 'กุมภาพันธ์ - มีนาคม' : 'กันยายน - ตุลาคม' ?></h5>
      <?php
        printPeriod($courses, (intval($_SESSION['term'][0]) - 1) * 2 + 1, $day_list, $mysqli);
       ?>
    </div>
    <div class="col s12 m6">
      <h5>คอร์ส <?php echo ($_SESSION['term'][0] == '1') ? 'เทอม 1' : 'เทอม 2' ?></h5>
      <?php
        printPeriod($courses, (intval($_SESSION['term'][0]) - 1) * 2 + 2, $day_list, $mysqli);
       ?>
    </div>
  </div>
  <div class="row">
  ...
  <div class='row'>
    <div class='col s12'>
      ...
      <p class='courseText1 hidden'>
        <b>คอร์ส <span id="course_period_1"></span> : <span id="course_name_1"></span>&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <p class='courseText2 hidden'>
        <b>คอร์ส <span id="course_period_2"></span> : <span id="course_name_2"></span>&nbsp;&nbsp;&nbsp;&nbsp;
        <span id="course_section_2"></span></b><br/>
        <b>ระยะเวลาคอร์ส: </b><span id="course_start_2"></span> - <span id="course_end_2"></span><br/>
        <b>วันที่เรียน: </b><span id="course_date_2"></span>&nbsp;&nbsp;&nbsp;&nbsp;
        <b>ห้องเรียน: </b><span id="course_room_2"></span><br/>
        <b>เวลาเรียน: </b><span id="course_time_2"></span><br/>
        <br/>
      </p>
    </div>
  </div>

  <form class="col s12" action="backend/reserve_seat_normal.php" method="post" enctype="multipart/form-data" id="slip-form">
    <hr/>
    <br/>
    ...
    <h6 id="pre-submit-text" class="col s12 red-text text-lighten-1">
      *กรุณาอัพโหลดหลักฐานการชำระเงินเพื่อนยืนยันการลงทะเบียนเรียน
      </br>
...
<div id="confirm_current" class="modal">
  <div class="modal-content">
    ...
    <b>คอร์ส <span id="course_period_confirm"></span> : <span id="course_name_confirm"></span>&nbsp;&nbsp;&nbsp;&nbsp;
