...
$(function () {
  $('.modal').modal({
      ...
      complete: function() {
        if(reserveComplete) {
          ...
          course_detail[period] = current_course_detail
  ...
  $('#submit-course').on('click', function () {
    ...
    if (course_detail[0] == undefined || course_detail[1] == undefined) {
...
function setPaymentSelect(){
  ...
  if(course_detail[1] != undefined){
    array.push(course_detail[1]['id'])
  }
...
function updateCourseStudent(force_ajax = [false, false]){
  if(force_ajax[0]) ajaxCourse1 = false;
  if(force_ajax[1]) ajaxCourse2 = false;
  ...
  if(course_seeing[1] != undefined && !ajaxCourse2){
    ajaxCourse2 = true;
    $.ajax('backend/ajax_course_std_count.php', {
      method: 'POST',
      data: { subjectid: course_seeing[1] },
      success: function ( res ){
        res = JSON.parse(res);
        res.forEach(function(o){
          $('a[data-courseid='+ o['courseid'] +']').find('span[student-amount]').text(o['count']);
